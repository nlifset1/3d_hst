...
for gal in lst_gal:
    for item in data_fast_flagged:
        if item['id'] == gal:
            lst_age.append(item['lage'])
            lst_nth.append(nth_nearest(gal, 3))
            break

#plotting lsfr vs nth nearest#
pylab.scatter(lst_age, lst_nth)

pylab.suptitle('Log Age vs Galaxy Number Density of All Redshifts', fontsize=23)
pylab.xlabel('Log Age', fontsize=16)
