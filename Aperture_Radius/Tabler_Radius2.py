...
#creating a function that takes a galaxies four count lists and makes them density lists#
def Density(gal_id, gal_field):
    #pulling data and initializing lists#
    ...
    #for each radius, calculating the density based on the counts in aperture radius#
...
#creating a similar function to Density, but for the average of four random points#
def Density_rand(gal_id, gal_field):
    #pulling redshift and field of given galaxy for picking random points#
    gal_info = gal_info = data_flagged[(data_flagged['id'] == gal_id) & (data_flagged['field'] == gal_field)]
    lst_r = 10**np.linspace(1.2,3.6,13)
    lst_dens1 = []
    lst_dens2 = []
    ...
    lst = [[],[],[],[]]
    #averaging and calculating densities#
...
def main():
    ...
    #initializing lists#
    #breakdown: lst_density [mass bin number] r [radius number]#
    lst_id = []
    lst_field = []
    lst_z = []
    lst_lmass = []
    ...
    #pulling necessary data and putting it into the lists#
    ...
    #breaking up lst_density's into a bunch of lists based on radius#
