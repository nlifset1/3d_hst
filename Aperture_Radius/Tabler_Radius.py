...
#creating a function that pulls in a list of counts per radius and creates a list of densities per radius#
def Density(gal_id, gal_field):
    #getting data and initializing lists#
    ...
    #for each radius, divide the count by the radius squared times pi#
...
def main():
        ...
        if gal[1] == 'AEGIS':
            ...
            if (gal_info['ra'] >= 0.59815/(math.pi/180)) and (gal_info['ra'] <= 0.602889/(math.pi/180)):
                if (gal_info['dec'] >= -0.091376/(math.pi/180)) and (gal_info['dec'] <= -0.090305/(math.pi/180)):
                    lst_gal.append(gal)
    #initializing lists#
    #lst_r and lst_rand are densities with the number representing radius#
    lst_id = []
    lst_field = []
    lst_z = []
    lst_lmass = []
    ...
    #getting necessary data and putting it into each list#
